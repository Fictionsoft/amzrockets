...
class ProductsController extends AppController {
    ...
    public function __builtContentWhere(){
        ...
        if(!empty($filter) or !empty($category_id)){
            $conditions = array(
                ...
                array('Product.model LIKE' => '%' . $filter . '%'),
     ...
     * @param null $slug
     */
    public function buy_now($id = null) {}
    ...
    public function details($id){
        ...
        if($this->request->is('post')){
            ...
            $this->__sendEmail(
                ...
                'Coupon',
                'coupon',
                array('email' => $email, 'coupon' => $product['Product']['coupon_code'], 'store_link' => $product['Product']['store_link'], 'site_name' =>$this->Common->settings('site_name') )
            );
    ...
    public function create_product_visitor($id,$mail_required = null){
        $this->loadModel('ProductVisitor');
        $this->ProductVisitor->save(array('product_id'=>$id));
